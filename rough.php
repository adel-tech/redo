...
if (isset($_POST['newName']) || isset($_POST['newCode']) || isset($_POST['newAge']) || isset($_POST['newDate']) || isset($_POST['enter']) || isset($_POST['newContact'])) {
  ...
  if ($res) {
    echo "<h4>"."successfully recorded"."</h4>";
  }
  else {
    echo "<h4>"."unsucessful"."</h4>";
  }
  echo "<form method='post'> <button name='display' formaction='all.php'> Back </button> </form>";
...
/*if (isset($_POST['update'])) {
  ...
  $newAge = $_POST['new-age'];
  $newContact = $_POST['new-contact'];
  $newGender = $_POST['new-gender'];



  $updateQuery = "UPDATE patients SET name=$newName,code=$newCode,age=$newAge,contact=$newContact,gender=$newGender WHERE no=$no; ";
 ...
 <?php
/* if (isset($_GET['enter'])) {
   if (isset($_GET['c-details'])) {
     $no=$_GET['no'];
     $newDetails=$_GET['c-details'];

     $add = "INSERT INTO redo (consultation) VALUES ('$newDetails') WHERE no='$no';";

     $result = mysqli_query($conn,$add);
     if ($result) {
          echo "patient details were recorded successfully";
   }
   else {
     echo "failed";
   }
...
UPDATE redo SET name='$newName',age='$newAge',date='$newDate',code='$newCode',consultation='$newDetails' WHERE no='$no'
