...
<script type="text/javascript">
  $(document).ready(function() {
    $("#consultation-search").keyup(function () {
      var txt=$(this).val();
      if (txt != ' ') {
        $.ajax({
          url:'consultation_fetch.php',
          method: 'post',
          data:{search:txt},
          success: function (data) {
            $("#consultation-results").html(data);
          }
        });
      }
      else {
        $("#consultation-results").html(" ");
      }
    });
  });
</script>
...
if (isset($_POST['list'])) {
 ...
 if ($numberOfResults > 0) {
   ...
   ?>
   <?php
   echo "<h4>"."There are"." ".$numberOfResults." "."patients"."<br>"."</h4>";
  // echo "<button type='submit' name='search' formaction='search.php'>Search</button>"."<br>";

   echo "<p><button><a href= 'index.php'>Back to Registration</a><br></button><p>";
?>
